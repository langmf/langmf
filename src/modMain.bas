...
Sub Parse_Type(txtCode As String)
    ...
    Do
        ...
        For a = 0 To UBound(txt)
            ...
            If ok Then
                ...
                Case "string"
                    If cnt = 0 And bnd <> 2 Then
                        ...
                        If bnd Then
                            ...
                            aPtr = aPtr & "s = " & ofs & "  :  For i = 0 To " & uds & "  :  [ХWRAP].PLong([ХOFS] + s) = StrPtr(" & sVar & "(i), True)  :  s = s + 4  :  Next  :  "
                        Else
                            aDim = aDim & "  Dim " & sVar & vbCrLf
                            aPtr = aPtr & "[ХWRAP].PLong([ХOFS] + " & ofs & ") = StrPtr(" & sVar & ", True)  :  "
                        End If
                        ofs = ofs + 4 * (uds + 1)
                    Else
                ...
                Case Else
                    If bnd Then
                        aDim = aDim & "  Dim " & sVar & "(" & uds & ")" & vbCrLf
                        iTxt = iTxt & "s = " & ofs & "  :  For i = 0 To " & uds & "  :  Set " & sVar & "(i) = New " & sType & "  :  [ХENUM].Add Array(" & sVar & "(i), s)  :  s = s + " & Types(sType) & "  :  Next  :  "
                    Else
                        aDim = aDim & "  Dim " & sVar & vbCrLf
                        iTxt = iTxt & "Set " & sVar & " = New " & sType & "  :  [ХENUM].Add Array(" & sVar & ", " & ofs & ")  :  "
                    End If
        ...
        oTxt = oTxt + "  Public Property Get [ХSIZE]()  :  [ХSIZE] = " & ofs & "  :  End Property" + vbCrLf
        oTxt = oTxt + "  Public Property Get [ХDATA]()  :  [ХDATA] = [ХWRAP].PBuffer([ХOFS], " & ofs & ")  :  End Property" + vbCrLf
        oTxt = oTxt + "  Public Property Let [ХDATA](v) :  [ХWRAP].PBuffer([ХOFS], " & ofs & ") = v  :  End Property" + vbCrLf
