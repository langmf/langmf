...
Function Parse_TypeSub(vOffset As Long, sVar As String, ByVal sType As String, ByVal bnd As Long, ByVal uds As Long, ByVal sz As Long, ByVal tp As Long) As String
    ...
    sz2 = sz:       sz = Abs(sz):       sArg = sArg & "[ХOFS] + " & vOffset
    ...
    sPrp = "[ХWRAP].P" & sType & "(" & sArg & ")":          vOffset = vOffset + sz * (uds + 1)
...
Sub Parse_Type(txtCode As String)
    ...
    Do
        ...
        If Mts.Count Then
                    ...
                    If mts1.Count > 0 Then
                        ...
                        Select Case LCase$(sType)
                            ...
                            Case "string"
                                If cnt = 0 And bnd <> 2 Then
                                    ...
                                    If bnd Then
                                        ...
                                        aPtr = aPtr & "s = " & vOffset & "  :  For i = 0 To " & uds & "  :  [ХWRAP].PLong([ХOFS] + s) = StrPtr(" & sVar & "(i), True)  :  s = s + 4  :  Next  :  "
                                    Else
                                        aDim = aDim & "  Dim " & sVar & vbCrLf
                                        aPtr = aPtr & "[ХWRAP].PLong([ХOFS] + " & vOffset & ") = StrPtr(" & sVar & ", True)  :  "
                                    End If
                            ...
                            Case Else
                                If bnd Then
                                    aDim = aDim & "  Dim " & sVar & "(" & uds & ")" & vbCrLf
                                    iTxt = iTxt & "s = " & vOffset & "  :  For i = 0 To " & uds & "  :  Set " & sVar & "(i) = New " & sType & "  :  [ХENUM].Add Array(" & sVar & "(i), s)  :  s = s + " & Types(sType) & "  :  Next  :  "
                                Else
                                    aDim = aDim & "  Dim " & sVar & vbCrLf
                                    iTxt = iTxt & "Set " & sVar & " = New " & sType & "  :  [ХENUM].Add Array(" & sVar & ", " & vOffset & ")  :  "
                                End If
            ...
            oTxt = oTxt + "Class " + .SubMatches(1) + vbCrLf                                                       'allow symbol  Х ¶ § Ђї ЛЫ
            oTxt = oTxt + "  Dim [ХWRAP], [ХENUM], [ХOFS]" + vbCrLf
            oTxt = oTxt + aDim + rTxt + vTxt + vbCrLf
            oTxt = oTxt + "  Public Function Class_New(v)   :  [ХWRAP].SetData(v)  :  End Function" + vbCrLf
            oTxt = oTxt + "  Public Property Get [ХSIZE]()  :  [ХSIZE] = " & vOffset & "  :  End Property" + vbCrLf
            oTxt = oTxt + "  Public Property Get [ХDATA]()  :  [ХDATA] = [ХWRAP].PBuffer([ХOFS], " & vOffset & ")  :  End Property" + vbCrLf
            oTxt = oTxt + "  Public Property Let [ХDATA](v) :  [ХWRAP].PBuffer([ХOFS], " & vOffset & ") = v  :  End Property" + vbCrLf
            oTxt = oTxt + "  Public Property Let [ХPTR](v)  :  [ХWRAP].Ptr = v  :  End Property" + vbCrLf
            oTxt = oTxt + "  Public Default Property Get [ХPTR]()  :  [ХPTR] = [ХWRAP].Ptr(True) + [ХOFS]  :  " + IIF(Len(aPtr), "Dim i,s  :  ", "") + aPtr + "End Property" + vbCrLf
            oTxt = oTxt + "  Private Sub Class_Initialize()  :  Set [ХENUM] = Sys.NewCol  :  " + IIF(Len(iTxt), "Dim i,s  :  ", "") + iTxt + "End Sub" + vbCrLf
...
Function Parse_Modify(txtCode As String, Optional txtConv As Variant, Optional ByVal Flags As Long) As String
    ...
    If IsMissing(txtConv) Then
        txtConv = Array("‘ункци(€|и)", "Function", "ѕроцедур(а|ы)", "Sub", "¬ызвать", "Call", " онстанта", "Const", _
        "ѕеременна€", "Dim", "ѕереопределить", "Redim", " омментарий", "Rem", " онец ÷икла", "Loop", "÷икл", "Do", _
        " онец ѕока", "Wend", "ѕока", "While", "ƒл€", "For", "ѕеребора", "Each", "ƒальше", "Next", "≈сли", "If", _
        "“огда", "Then", "»наче", "Else", "»наче≈сли", "ElseIf", "—войств(о|а)", "Property", "ѕрисвоить", "Let", _
        "”становить", "Set", "ѕолучить", "Get", "¬ыбора?", "Select", "”словие", "Case", "ѕо условию", "Until", _
        "—в€зать", "With", "»стина", "True", "Ћожь", "False", "Ќуль", "Null", "ѕусто", "Empty", "—тереть", "Erase", _
        "ќчистить", "Nothing", "¬ыход", "Exit", "≈сть", "Is", "ƒо", "To", "Ўаг", "Step", "„астн(а€|ое)", "Private", _
        "ѕубличн(а€|ое)", "Public", "Ќовый", "New", " ак", "As", "“ип", "Type", " онец", "End", "¬озврат", "Return", _
        "ѕродолжить", "Continue", "—ообщение", "MsgBox", "—оздатьќбъект", "CreateObject", "ѕолучитьќбъект", "GetObject", _
        "а", "a", "б", "b", "в", "v", "г", "g", "д", "d", "е", "e", "Є", "jo", "ж", "zh", "з", "z", "и", "i", "й", "j", _
        "к", "k", "л", "l", "м", "m", "н", "n", "о", "o", "п", "p", "р", "r", "с", "s", "т", "t", "у", "u", "ф", "f", _
        "х", "x", "ц", "c", "ч", "ch", "ш", "sh", "щ", "sch", "ъ", "qi", "ы", "y", "ь", "qu", "э", "e", "ю", "yu", "€", "ya")
    End If
